...
while True:
    ...
    if response.ok and vacancies:
        for vacancy in vacancies:
            ...
            vacancy_data['_id'] = ''.join([en for en in link if en.isdigit()])  # добавляем id каждой вакансии
            # vacancy_list.append(vacancy_data)

            try:
                db.vacancies.insert_one(vacancy_data)      # добавляем вакансию в базу данных
...
def salary(value, collection):
    n = 0
    for item in collection.find({'$or': [{'salary_min': {'$gt': value}}, {'salary_max': {'$gt': value}}]}):
        pprint(item)
        n += 1
    print(f'Найдено {n} вакансий с зарплатой выше {value}')


value_salary = int(input('Укажите необходимую зарплату: '))
salary(value_salary, db.vacancies)
